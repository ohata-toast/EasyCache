...
* レプリケーショングループのバージョンが、バックアップが作られたレプリケーショングループのバージョン以上のレプリケーショングループにのみ復元できます。
...
4. **マトリックス条件**と**イベント条件**に必要な設定を決定するのが難しい場合は、**アラームテンプレート**で提供する基本テンプレートを利用することを検討してください。
...
|             | 既存レプリケーショングループ復元 | 開始、データファイルダウンロード失敗、損傷したファイルまたはサポートしないファイル形式、ノード再起動失敗、レプリケカ同期失敗、終了 |
|             |  HA設定更新 | 開始、失敗、終了 |
|             | マスター変更 | 開始、失敗、終了 |
