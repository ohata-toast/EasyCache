...
> [주의] EasyCache는 Redis를 기반으로 한 캐시 상품으로써 웹콘솔 페이지에서 제공하는 자동 및 수동 백업 기능 외 데이터 영속성을 위한 기능은 제공하지 않습니다. 따라서 재시작이나 예상하지 못한 장애 등으로 인한 데이터 유실 발생 시, 자동 백업이나 수동 백업을 통한 백업 데이터가 없을 경우 복구를 할 수 없습니다.
