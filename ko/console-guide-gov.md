...
### 엔진 버전 업그레이드
* Redis 버전 5인 기존 복제 그룹을 가장 최신 Redis 버전으로 업그레이드할 수 있습니다.
* 엔진 버전 업그레이드를 실행하려면 낮은 Redis 버전을 사용중인 대상 복제 그룹을 선택한 후 기타 액션 버튼(⋯)을 클릭하고 **엔진 버전 업그레이드**를 클릭합니다.
* **엔진 버전 업그레이드** 대화 상자에서 엔진 버전 업그레이드 시 적용될 설정 프로필을 선택할 수 있습니다.
...
* Redis 버전을 업그레이드하려는 기존 복제 그룹에 기본 설정 프로필 이외의 사용자 설정 프로필이 적용되어 있는 경우, 기존과 유사하면서 업그레이드할 버전에 맞는 설정 프로필을 적용하려면 **프로필 설정**에서 알맞는 버전의 사용자 설정 프로필을 미리 생성한 후 버전 업그레이드 시 미리 생성한 설정 프로필을 선택해야 합니다.
...
* 엔진 버전 업그레이드에 실패하면 **복제 그룹 > ⋯ > 버전 업그레이드**를 클릭해서 버전 업그레이드를 다시 실행할 수 있습니다. 

### OS 버전 업그레이드
* 보안상의 이슈 등 고객의 필요에 따라 복제 그룹을 구성하는 노드에 사용중인 OS의 버전을 이지캐시 상품이 제공하는 가장 최신 버전(2024.08.13 기준 Ubuntu 22.04)으로 업그레이드할 수 있습니다.
* OS 버전 업그레이드를 실행하려면 OS 버전이 낮은 노드가 속한 복제 그룹을 선택한 후 기타 액션 버튼(⋯)을 클릭하고 **OS 버전 업그레이드**를 클릭합니다.
* **OS 버전 업그레이드** 대화 상자에서 OS 버전 업그레이드를 진행할 노드를 선택할 수 있습니다.
...
* **replication** 타입의 복제 그룹의 경우
    * 마스터노드가 아닌 복제 노드만 OS 버전 업그레이드 수행이 가능합니다.
    * 마스터 노드를 업그레이드하고 싶다면, 먼저 마스터 변경 기능을 통해 마스터를 변경한 뒤 구 마스터 노드를 선택하여 OS 업그레이드를 진행하여야합니다.
    * OS 버전 업그레이드를 진행하려는 노드가 속한 복제 그룹의 HA 노드가 최신 OS 버전을 사용중이지 않다면, HA 노드의 OS 업그레이드도 같이 진행되므로 경우에 따라 시간이 좀 더 오래 걸릴 수 있습니다.
...
3. 알람 발생 조건에는 **메트릭 조건**과 **이벤트 조건**, 2가지가 있습니다.
    ...
    * **메트릭 조건**: 캐시 인스턴스에서 수집한 각종 성능 지푯값(모니터링 항목 참조)을 이용해 알람 조건을 지정하며 다음과 같은 조건을 지정할 수 있습니다.
        ...
        * 감시 항목의 성능 지푯값과 임곗값을 비교하여 조건이 충족하는지 판단합니다. 조건이 지속 시간 이상 연속해서 충족한다면 알림을 발송합니다. 예를 들어, CPU 사용률의 임곗값이 90% 이상이고 지속 시간이 5분이라면, 해당 알림 그룹과 연동된 노전의 CPU 사용률이 90% 이상인 상태가 5분 이상 지속되었을 때 사용자 그룹에 정의된 사용자들에게 알림을 보냅니다. 만약 CPU 사용률이 90% 이상이 되어도, 5분 이내에 90% 미만으로 떨어지면 알림이 발생하지 않습니다.
